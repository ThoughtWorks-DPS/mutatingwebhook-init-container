...
func main() {
	...
	// if theList, err := kubeClient.AdmissionregistrationV1().MutatingWebhookConfigurationsList()
	// 	err != nil {
	// 	panic(err)
	// }

	if _, err := kubeClient.AdmissionregistrationV1().MutatingWebhookConfigurations().Create(context.Background(), mutateconfig, metav1.CreateOptions{})
		err != nil {
		panic(err)
	}
}
